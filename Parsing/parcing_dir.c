...
int check_diection_texter(t_game *game, char *str)
{
    ...
    {
        if (!parce_and_filling_line(&game->config->no_texture, str + 2, &game->config->check_duplicate[0], "NO"))
            return (0);
    }
    else  if (!ft_strncmp(str, "SO", 2))
    {
        if (!parce_and_filling_line(&game->config->so_texture, str + 2, &game->config->check_duplicate[1], "SO"))
            return (0);
    }
    else  if (!ft_strncmp(str, "WE", 2))
    {
        if (!parce_and_filling_line(&game->config->we_texture, str + 2, &game->config->check_duplicate[2], "WE"))
            return (0);
    }
    else  if (!ft_strncmp(str, "EA", 2))
    {
        if (!parce_and_filling_line(&game->config->ea_texture, str + 2, &game->config->check_duplicate[3], "EA"))
            return (0);
    }
    return (1);
}
